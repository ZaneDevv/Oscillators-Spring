...
In physics, this formula works just nice, but in simulations it will never stop. Thus, we need to add a damping parameter to reduce the velocity of the object in order to help it from moving without a stop.
...
\[
    v_{n+1} = v_n - \Delta t \frac{\beta v_n + k(p - r)}{m}
\]
\[
    x_{n+1} = x_n + v_n \Delta t
\]

We start our simulation and we will see such a great motion in our body, but it could be even better. 
...
Let us start with two fundamental laws in physics: damped Hooke's law and 2nd Newton's law:
...
Using these names, the formula we had so far turns into:

\[
    a + 2 \omega \zeta v + \omega^2 x = 0
\]

From basic physics, we know that the velocity is nothing but the derivative of the position with respect to time and, acceleration, is the second derivative of the position with respect to time:
...
The first possibility is the simplest one because, since $\zeta^2 - 1 = 0$, $\sqrt{\zeta^2 - 1} = 0$, the equation has only one result:
